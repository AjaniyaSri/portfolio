        ...
        {projects.map((project, index) => {
          return (
            <motion.article
              ...
              whileHover={{ y: -6 }}
              className="group relative overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 transition-all duration-300 hover:border-pink-300/80"
            >
              <div className="relative h-56 w-full overflow-hidden">
                ...
                <div className="absolute top-4 left-4 z-10">
                  <div className="flex items-center gap-2 rounded-full border border-pink-300/70 bg-slate-900/80 px-3 py-1.5 text-xs font-medium text-slate-100">
              ...
              <div className="relative p-6 text-sm text-slate-300">
                <h3 className="text-xl font-semibold leading-tight text-white transition-colors duration-200 group-hover:text-pink-200">
                  {project.title}
                </h3>
                {project.role && (
                  <p className="mt-1 text-xs font-medium uppercase tracking-wide text-pink-300">
                    {project.role}
                  </p>
                )}
                <ul className="mt-3 space-y-1.5">
                  {project.points.map((point) => (
                    <li key={point} className="flex gap-2">
                      <span className="mt-1 h-1.5 w-1.5 flex-none rounded-full bg-pink-400 group-hover:bg-sky-300" />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
                {project.link && (
                  <div className="mt-5 flex flex-wrap gap-3">
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="group/btn inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-pink-400 to-sky-400 px-5 py-2.5 text-xs font-semibold text-slate-950 transition-colors duration-200 hover:from-pink-300 hover:to-sky-300"
                    >
                      <Github
                        size={14}
                    ...
                    <motion.a
                      ...
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      className="group/btn inline-flex items-center gap-2 rounded-full border border-slate-700 bg-slate-900/70 px-5 py-2.5 text-xs font-semibold text-slate-200 transition-colors duration-200 hover:border-pink-300 hover:text-pink-200"
                    >
                      <ExternalLink
                        size={14}
